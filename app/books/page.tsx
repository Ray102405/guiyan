...
export default function BooksPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      toast.loading("上传中...")
      const res = await fetch("/api/backend/api/books/upload", {
        method: "POST",
        body: formData,
      })
      toast.dismiss()
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-28">
        ...
        {books.length === 0 ? (
          <div className="mt-16 flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-muted-foreground/50">书架上还没有书</p>
            <button
              onClick={() => fileRef.current?.click()}
              className="rounded-lg border border-border bg-card px-4 py-2 text-xs text-muted-foreground transition-colors hover:bg-muted"
            >
              上传你的第一本书
            </button>
          </div>
